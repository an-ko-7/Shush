...
export default function DataViewer({ data }: Props) {
  ...
  const elapsed = data.elapsed ?? data[1];
  const segments = data.segments ?? output?.chunks ?? [];
  const srt = data.srt ?? "";
      ...
      <Tabs defaultValue="timestamps" className="w-full h-full">
        <div className="w-full justify-between flex items-center">
          ...
          <span className="sm:inline hidden">Finished on a <span className="font-medium">NVIDIA A10G</span> in{" "}</span>
            <span className="font-medium">~{elapsed.toFixed(2)}s</span>
          </div>
        </div>
        <TabsContent
        ...
        <TabsContent
          ...
          value="timestamps"
        >
          {segments.map((chunk: any, index: number) => (
        ...
        >
          ...
          <pre className="whitespace-pre-wrap">{srt || "No SRT available."}</pre>
        </TabsContent>
        <TabsContent
